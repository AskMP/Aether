...
(function () {
    ...
    /***
    * Æther module for websocket direct connection
    */
    var WebSockets = function () {
        ...
        this.init = function () {
            ...
            Æther.log.notify('👀  WebSockets Initializing');
        };
        ...
        this.initializeConnection = (connection) => {
            ...
            Æther.log.notify('👀  New WebSocket connected');
        ...
        this.processMessage = (message) => new Promise((resolve, reject) => {
            if (message) {
                ...
                if (message.broadcast === true) {
                    Æther.emit(message.evt, message.data);
                }
                if (message.trackable === true) {
                    Æther.analytics.track(message.evt, message.data, 'websocket');
                }
            }
            resolve();
        });
        ...
        this.rejectMessage = function (err, connection) {
            ...
            Æther.log.error('👀  Invalid WebSockets connection: ' + err);
        };
        ...
        this.errored = (err) => {
            Æther.log.error('👀  WebSocket Error: ' + JSON.stringify(err));
        };
                           
        this.startListening = new Promise((resolve, reject) => {
            this.connections.listen(config.port);
            Æther.log.notify('👀  Listening for WebSockets on port ' + config.port);
            resolve();
        });
