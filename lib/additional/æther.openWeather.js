...
(function () {
    ...
    var OpenWeather = function () {
        ...
        this.init = () => new Promise((resolve, reject) => {
            ...
            Aether.log.notify('🌦  Initializing OpenWeather module');
        ...
        this.getCurrent = () => new Promise((resolve, reject) => {
            ...
            Aether.delegate.getJSON(apiEndpoints.current, { id : config.cityID, appid : config.appID, units : 'metric' })
                .then(data => {
                    ...
                    Aether.log.notify('⛅  current conditions captured as of ' + this.currentConditions.lastUpdate);
                    resolve();
                })
                .catch(err => console.log(err));
        });
        ...
        this.getForcast = () => new Promise((resolve, reject) => {
            Aether.log.notify('🌦  Capturing forcast');
            Aether.delegate.getJSON(apiEndpoints.forcast, { id : config.cityID, appid : config.appID, units : 'metric' })
                .then(data => {
                    ...
                    Aether.log.notify('🌦  forcast conditions captured as of ' + this.forcastConditions.lastUpdated);
                })
                .then(resolve)
                .catch(err => console.log(err));
        });
